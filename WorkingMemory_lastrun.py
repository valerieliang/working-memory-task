...
expName = 'WorkingMemory'  # from the Builder filename that created this script
expInfo = {
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\TheAn\\OneDrive\\Documents\\GitHub\\working-memory-task\\WorkingMemory_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    text_norm = visual.TextStim(win=win, name='text_norm',
        text="In this experiment, we want to study how precisely people remember colors. On each trial, you will see a number of squares presented simultaneously in different colors. Your job is to try to remember the color of each square as precisely as you can. After about half a second, the squares will disappear. Keep them in mind. Then, the positions of one of the squares will be probed, and your job is to report the color of the square that was just in that position on a color wheel. We’ll do 10 practice trials to get you used to everything. Feel free to ask questions, if you have any. Do your best, and if you feel uncertain, just guess.\n\nIf you are ready to proceed, press 'space'.",
        font='Arial',
    ...
    slider = visual.Slider(win=win, name='slider',
        startValue=0.5, size=(1.0, 0.1), pos=(0, -0.4), units=win.units,
        labels=None, ticks=(0,1), granularity=0.0,
        style='slider', styleTweaks=(), opacity=0.0,
        labelColor=[0.0000, 0.0000, 0.0000], markerColor=[0,0,0], lineColor=[0.0000, 0.0000, 0.0000], colorSpace='rgb',
        font='Open Sans', labelHeight=0.05,
        flip=False, ori=0.0, depth=-10, readOnly=False)
    colobar = visual.ImageStim(
        win=win,
        name='colobar', 
    ...
    Block1 = data.TrialHandler(nReps=1.0, method='random', 
        ...
        trialList=data.importConditions('WorkingMemory_block1.xlsx'),
    ...
    for thisBlock1 in Block1:
        ...
        slider.reset()
        # keep track of which components have finished
        trialComponents = [fixation, rect1, rect2, rect3, rect4, rect5, rect6, rect7, rect8, probe, slider, colobar]
        ...
        while continueRoutine:
            ...
            if rect1.status == NOT_STARTED and tThisFlip >= 2-frameTolerance:
                # keep track of start time/frame for later
            ...
            if rect1.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > rect1.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if rect2.status == NOT_STARTED and tThisFlip >= 2-frameTolerance:
                # keep track of start time/frame for later
            ...
            if rect2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > rect2.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if rect3.status == NOT_STARTED and tThisFlip >= 2-frameTolerance:
                # keep track of start time/frame for later
            ...
            if rect3.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > rect3.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if rect4.status == NOT_STARTED and tThisFlip >= 2-frameTolerance:
                # keep track of start time/frame for later
            ...
            if rect4.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > rect4.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if rect5.status == NOT_STARTED and tThisFlip >= 2-frameTolerance:
                # keep track of start time/frame for later
            ...
            if rect5.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > rect5.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if rect6.status == NOT_STARTED and tThisFlip >= 2-frameTolerance:
                # keep track of start time/frame for later
            ...
            if rect6.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > rect6.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if rect7.status == NOT_STARTED and tThisFlip >= 2-frameTolerance:
                # keep track of start time/frame for later
            ...
            if rect7.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > rect7.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if rect8.status == NOT_STARTED and tThisFlip >= 2-frameTolerance:
                # keep track of start time/frame for later
            ...
            if rect8.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > rect8.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if probe.status == NOT_STARTED and tThisFlip >= 3.4-frameTolerance:
                # keep track of start time/frame for later
            ...
            if slider.status == NOT_STARTED and tThisFlip >= 3.4-frameTolerance:
                # keep track of start time/frame for later
            ...
            # if colobar is starting this frame...
            if colobar.status == NOT_STARTED and tThisFlip >= 3.4-frameTolerance:
                # keep track of start time/frame for later
                colobar.frameNStart = frameN  # exact frame index
                colobar.tStart = t  # local t and not account for scr refresh
                colobar.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(colobar, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'colobar.started')
                # update status
                colobar.status = STARTED
                colobar.setAutoDraw(True)
            
            # if colobar is active this frame...
            if colobar.status == STARTED:
                # update params
                pass
